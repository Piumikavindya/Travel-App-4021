...
export default function UpdateJourney() {
  ...
  useEffect(() => {
    ...
    axios
      ...
      setEvents(response.data.Events.map((value) => ({ value, label: value }))); // <-- Close this map function properly
      setResorts(response.data.Resorts.map((value) => ({ value, label: value }))); // <-- Close this map function properly
      setPackages(response.data.Packages.map((value) => ({ value, label: value }))); // <-- Close this map function properly
      })
      .catch((err) => {
        console.error(err);
      });
